const getLocalStorage = () => JSON.parse(localStorage.getItem('db_categories')) ?? [];

const setLocalStorage = (dbCategories) => localStorage.setItem('db_categories', JSON.stringify(dbCategories));
...
const editDelete = (e) => {
  if (e.target.type == 'button') {
    ...
    if (action == 'edit') {
      ...
      const response = confirm(`deseja realmente excluir o produto ${categories.nameCategories}`);
      if (response) {
